...
{
    float a4 = max(Pow4(roughness), 0.00001f);
    float3 V;
...
float3 SampleHemisphereBrute(float3 normal)
{
    ...
    float invDim = 1.f / gCubeMapInSize;

    for (uint face = 0; face < 6; ++face)
    {
        for (uint y = 0; y < gCubeMapInSize; ++y)
        {
            for (uint x = 0; x < gCubeMapInSize; ++x)
            {
...
void EquirectangularToCubeMapCS(uint3 DispatchThreadID : SV_DispatchThreadID, uint3 GroupID : SV_GroupID)
{
    ...
    if(gSampleCount == 1) dir.x = 1.f - dir.x;
