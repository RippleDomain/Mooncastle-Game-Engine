...
//UV is the screen-space UV coordinate of the pixel.
//Depth is the z-buffer depth.
...
float4 UnprojectUV(float2 uv, float depth, float4x4 inverse)
{
    ...
    float4 cClip = float4(float2(uv.x, 1.f - uv.y) * 2.f - 1.f, depth, 1.f);

    // 3D-space position (before perspective divide).
    float4 position = mul(inverse, cClip);
...
{
    //Converts to normalized texture coordinates.
