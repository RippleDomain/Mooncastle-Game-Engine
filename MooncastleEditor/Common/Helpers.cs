...
namespace MooncastleEditor
{
    ...
    static class BitmapHelper
    {
        ...
        public static byte[] GenerateThumbnail(BitmapSource image, int maxWidth, int maxHeight)
        {
        ...
        public static BitmapSource ImageFromSlice(Slice slice, DXGI_FORMAT sliceFormat, bool isNormalMap = false)
        {
            var data = slice.RawContent;
            var bytesPerPixel = data.Length / (slice.Width * slice.Height);
            var bytesPerChannel = BytesPerChannel(sliceFormat);
            ...
            else if (bytesPerPixel == 4)
            {
                ...
                {
                    int offset = 0;
                    float[] dataFloats = data.GroupBy(x => offset++ / bytesPerChannel)
            ...
            else if (bytesPerPixel == 2)
            {
                ...
                {
                    int offset = 0;
                    Half[] dataFloats = data.GroupBy(x => offset++ / bytesPerChannel)
