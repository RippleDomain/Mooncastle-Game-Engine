...
namespace MooncastleEditor.Content
{
    ...
    public partial class ConfigureTextureImportSettingsView : UserControl
    {
        ...
        private void ImageSource(object sender, ListBox source, Action<TextureProxy, List<TextureProxy>> action)
        {
            ...
            foreach (TextureProxy proxy in source.SelectedItems)
            {
                selection.Add(proxy);
            }
        ...
        {
            ImageSource(sender, textureListBox, (target, selection) =>
            {
                var vm = (DataContext as ConfigureImportSettings).TextureImportSettingsConfigurator;

                foreach (var proxy in selection)
                {
        ...
        {
            ImageSource(sender, imageSourcesListBox, (target, selection) =>
            {
                var vm = (DataContext as ConfigureImportSettings).TextureImportSettingsConfigurator;
